...
use crate::infrastructure::integrations::github::GithubClient;
use crate::infrastructure::integrations::github::oauth::GithubOAuthClient;
...
pub struct ApplicationState {
    ...
    pub integrations: ApplicationIntegrations,
    pub cache: Arc<RedisCache>,
    pub cipher: Arc<ReversibleCipher>,
}
...
pub struct ApplicationIntegrations {
    pub github_oauth_client: Arc<GithubOAuthClient>,
...
impl ApplicationState {
    pub fn new(db: DatabaseConnection) -> Self {
        ...
        let integrations = Self::build_integrations();

        let cache = Arc::new(RedisCache::new());

        let cipher = Arc::new(ReversibleCipher::new(
            &ENV.get("REVERSABLE_CIPHER_SECRET_KEY"),
        ));
    ...
    fn build_services() -> ApplicationServices {
        let task_tracker: Arc<dyn TaskTrackerService> = Arc::new(KaitenTaskTrackerService::new());

        let notifier: Arc<dyn NotificationService> =
            Arc::new(TelegramNotifierService::new(ENV.get("TELEGRAM_BOT_TOKEN")));
    ...
    fn build_repositories(db: Arc<DatabaseConnection>) -> ApplicationRepositories {
        let user_repository: Arc<dyn UserRepository> = Arc::new(MySQLUserRepository::new(db));
    ...
    fn build_integrations() -> ApplicationIntegrations {
