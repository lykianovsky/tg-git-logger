...
use crate::domain::task_tracker::service::{
    TaskTrackerColumnId, TaskTrackerMoveCardError, TaskTrackerService, TaskTrackerTaskId,
};
use crate::infrastructure::integrations::kaiten::{
    ...
    pub fn new() -> KaitenTaskTrackerService {
        Self {
            client: KaitenClient::new(
                KaitenClientBase(ENV.get("TASK_TRACKER_BASE")),
                KaitenClientToken(ENV.get("TASK_TRACKER_API_TOKEN")),
            ),
        }
    }
}
...
impl TaskTrackerService for KaitenTaskTrackerService {
    async fn move_card(
        &self,
        task_id: &TaskTrackerTaskId,
        column_id: &TaskTrackerColumnId,
    ) -> Result<(), TaskTrackerMoveCardError> {
        self.client
            .move_card(&task_id.0, &column_id.0)
            .await
            .map_err(|err| {
    ...
    fn create_task_link(&self, id: &TaskTrackerTaskId, text: &str) -> String {
        let link = ENV
            .get("TASK_TRACKER_PATH_TO_CARD")
            .replace("{id}", id.0.as_str());
