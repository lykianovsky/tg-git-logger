...
use crate::domain::user::repository::{
    AssignUserRoleException, BindGitHubException, CreateUserException,
    FindUserByGitHubLoginException, FindUserByIdException, FindUserByTgIdException, UserRepository,
};
use crate::domain::user::value_objects::{
    UserGithubAccount, UserGithubId, UserId, UserRole, UserTelegramAccount, UserTelegramId,
};
use crate::infrastructure::database::mysql::entities::{
    git_hub_accounts, roles, telegram_accounts, user_roles, users,
};
...
impl UserRepository for MySQLUserRepository {
    async fn create(&self, user: &User) -> Result<(), CreateUserException> {
        let txn = self
            ...
            .begin()
            .await
            .map_err(|_| CreateUserException::DbError("Transaction start failed".into()))?;
    ...
    async fn find_by_tg_id(&self, id: UserTelegramId) -> Result<User, FindUserByTgIdException> {
        let tg = telegram_accounts::Entity::find()
            ...
            .ok_or(FindUserByTgIdException::DbError(
                "Telegram user not found".into(),
            ))?;
    ...
    async fn find_by_github_login(
        &self,
        id: String,
    ) -> Result<User, FindUserByGitHubLoginException> {
        let gh = git_hub_accounts::Entity::find()
            ...
            .ok_or(FindUserByGitHubLoginException::DbError(
                "GitHub user not found".into(),
            ))?;
    ...
    async fn assign_role(
        ...
        user_id: UserId,
        role: UserRole,
    ) -> Result<(), AssignUserRoleException> {
        let role = roles::Entity::find()
            .filter(roles::Column::Name.eq(role.to_str()))
            .one(self.db.as_ref())
            .await
            .map_err(|e| AssignUserRoleException::DbError(e.to_string()))?
