...
impl TelegramBotDialogueAdminHealthPingDispatcher {
    ...
    > {
        dptree::entry()
            .branch(
                case![TelegramBotDialogueAdminState::HealthPingList]
                    .endpoint(handle_list_action),
            )
    ...
    > {
        dptree::entry()
            .branch(
                TelegramBotDialogueAdminHealthPingCreateDispatcher::message_branches(),
            )
            .branch(
                TelegramBotDialogueAdminHealthPingEditDispatcher::message_branches(),
            )
    }
    ...
    pub async fn show_list(
        ...
        let pings = match pings {
            ...
            Err(e) => {
                ...
                bot.edit_message_text(
                    chat_id,
                    message_id,
                    t!("telegram_bot.dialogues.admin.health_ping.load_error")
                        .to_string(),
                )
                .await?;
        ...
        let mut builder = MessageBuilder::new()
            .bold(
                &t!("telegram_bot.dialogues.admin.health_ping.title").to_string(),
            )
            .empty_line();
        ...
        if !pings.is_empty() {
            keyboard =
                keyboard.row::<TelegramBotAdminHealthPingAction>(vec![
...
async fn handle_list_action(
    ...
    match action {
        TelegramBotAdminHealthPingAction::Create => {
            ...
            bot.send_message(
                msg.chat().id,
                t!("telegram_bot.dialogues.admin.health_ping.enter_name")
                    .to_string(),
            )
            .await?;
        }
        ...
        TelegramBotAdminHealthPingAction::Edit => {
            ...
            bot.send_message(
                msg.chat().id,
                t!("telegram_bot.dialogues.admin.health_ping.select_for_edit")
                    .to_string(),
            )
