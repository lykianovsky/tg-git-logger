...
impl TelegramBotLoginCommandHandler {
    ...
    pub async fn execute(&self) -> Result<Message, RequestError> {
        return match CreateUserByTelegramUserUseCase::new(Arc::clone(
            &self.context.application_state.repositories.user,
        ))
        .execute(
            ...
            Ok(user) => {
                self.context
                    ...
                    .send_message(
                        ...
                        format!(
                            "You have been logged in successfully, user ID: {}",
                            user.id.0
                        ),
                    )
                    .await
