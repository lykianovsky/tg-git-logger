...
use crate::application::user::queries::get_my_pull_requests::response::GetMyPullRequestsResponse;
...
pub struct GetMyPullRequestsExecutor {
...
impl GetMyPullRequestsExecutor {
    pub fn new(
        user_socials_repo: Arc<dyn UserSocialAccountsRepository>,
        user_vc_accounts_repo: Arc<dyn UserVersionControlAccountsRepository>,
        user_connection_repositories_repo: Arc<dyn UserConnectionRepositoriesRepository>,
...
impl CommandExecutor for GetMyPullRequestsExecutor {
    type Command = GetMyPullRequestsQuery;
    type Response = GetMyPullRequestsResponse;
    type Error = GetMyPullRequestsError;
    ...
    async fn execute(&self, cmd: &Self::Command) -> Result<Self::Response, Self::Error> {
        let social = self
            ...
            .await
            .map_err(|_| GetMyPullRequestsError::UserNotFound)?;

        let vc = self
            .user_vc_accounts_repo
            .find_by_user_id(&social.user_id)
            .await
            .map_err(|_| GetMyPullRequestsError::NoGithubAccount)?;

        let token = self
            .reversible_cipher
            .decrypt(vc.access_token.value())
            .map_err(|e| GetMyPullRequestsError::GithubError(e.to_string()))?;

        let connections = self
            .user_connection_repositories_repo
            .find_by_user_id(social.user_id)
            .await
            .map_err(|e| GetMyPullRequestsError::DbError(e.to_string()))?;
        ...
        let prs = self
            .version_control_client
            .search_user_authored_open_prs(&token, &vc.version_control_login, &repos)
            .await
            .map_err(|e| GetMyPullRequestsError::GithubError(e.to_string()))?;

        Ok(GetMyPullRequestsResponse { prs })
    }
}
